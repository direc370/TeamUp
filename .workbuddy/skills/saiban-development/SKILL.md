...
1. V0.3 本地交互闭环：已完成。
2. GitHub 版本管理与基础单元测试：已完成。
3. 接入 Supabase 客户端及环境变量模板。
4. 实现邮箱注册、登录、退出和会话保持。
5. 建立 profiles、projects、applications、saved_projects 数据表及 RLS。
6. 将本地组队帖、收藏和申请切换为云端数据。
7. 增加表单安全校验、加载/错误/空状态和端到端测试。
8. 部署预览环境，完成移动端、权限和异常场景验收。
9. 配置生产环境、域名、监控、隐私说明和数据备份后小范围上线。
10. 上线后实现协作工作台：任务五态、接单、死线和异步简报。
11. 实现 task_events 与贡献度参考报告。
