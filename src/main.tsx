...
function App() {
  ...
  }), [posts, category, goal, time, query])
  ...
  const publishPost = (event: FormEvent<HTMLFormElement>) => {
    ...
    const skills = form.skills.split(/[,，、\s]+/).map((skill) => skill.trim()).filter(Boolean).slice(0, 5)
    if (form.title.trim().length < 4) {
      setFormError('项目名称至少填写 4 个字。')
      return
    }
    if (form.description.trim().length < 10) {
      setFormError('请用至少 10 个字说明队友缺口和协作要求。')
      return
    }
    if (!skills.length) {
      setFormError('请至少填写一个需要的技能。')
      return
    }
